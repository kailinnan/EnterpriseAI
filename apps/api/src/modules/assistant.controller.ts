...
export class AssistantController {
  ...
  @RequireScopes('chat:write') @Post('conversations/:id/messages') async chat(
    ...
    try {
      ...
      const sources = chunks
        ...
        .join('\n');
      const history =
        await db()`select role,content_json from messages where tenant_id=${p.tenantId} and conversation_id=${id} order by created_at desc limit 12`;
      ...
      for await (const delta of adapter.stream({
        ...
        messages: [
          ...
          ...history
            ...
            .map((row) => ({
              ...
              content: String((row.content_json as { text?: string }).text ?? ''),
            })),
          { role: 'user', content },
        ],
        ...
        answer += delta;
        if (first) {
          firstTokenLatency.observe((Date.now() - started) / 1000);
          first = false;
        }
      ...
      await db().begin(async (tx) => {
        await tx`insert into messages(tenant_id,conversation_id,role,content_json,token_usage_json,model_name,latency_ms,trace_id) values(${p.tenantId},${id},'assistant',${tx.json({ text: answer, citations })},${tx.json({ inputTokens, outputTokens })},${config.model_name},${latency},${req.traceId})`;
  ...
  @Post('conversations/:id/summarize') async summarize(
    ...
    const messages = await this.service.messages(p, uuid(id));
    const summary = messages
      .slice(-20)
      .map(
        (x) =>
          `${x.role}: ${String((x.content_json as { text?: string }).text ?? '').slice(0, 200)}`,
      )
      .join('\n')
      .slice(0, 4000);
    await db()`update conversations set summary=${summary} where tenant_id=${p.tenantId} and id=${id} and user_id=${p.userId}`;
    return { summary };
  }
}
