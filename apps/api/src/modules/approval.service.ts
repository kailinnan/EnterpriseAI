...
export class ApprovalService {
  ...
  async decide(
    ...
    if (!call) {
      ...
      return { status: existing.approval_status, output: existing.output_json, idempotent: true };
    }
    ...
    await db().begin(async (tx) => {
      ...
      await tx`update agent_runs set status=${result.ok ? 'completed' : 'failed'},output_json=${tx.json(toJsonValue(result))},completed_at=now() where tenant_id=${p.tenantId} and id=${call.agent_run_id}`;
    });
    await this.audit(p, id, 'executed', req);
    return { status: result.ok ? 'completed' : 'failed', output: result };
  }
